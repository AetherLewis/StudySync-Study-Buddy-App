...
const validateQuizPayload = (payload) => {
  ...
  const description = sanitizeString(payload.description) || "AI generated quiz";
...
const buildQuizPrompt = ({ text, topic, category, count }) => {
  const questionCount = Math.min(Math.max(Number(count) || DEFAULT_QUESTION_COUNT, 1), 10);
  const topicHint = sanitizeString(topic) ? `Topic: ${sanitizeString(topic)}\n` : "";
  const categoryHint = sanitizeString(category)
    ? `Preferred category: ${sanitizeString(category)}\n`
    : "";

  return `You are a rigorous educational assistant. Generate ${questionCount} multiple-choice quiz questions from the following study text. Respond with valid JSON only, using this exact schema:\n{\n  \"title\": \"...\",\n  \"description\": \"...\",\n  \"category\": \"...\",\n  \"questions\": [\n    {\"question\": \"...\", \"options\": [\"...\", \"...\", \"...\", \"...\"], \"correctAnswer\": 0, \"explanation\": \"...\"}\n  ]\n}\n${topicHint}${categoryHint}Study text:\n\"\"\"${sanitizeString(text)}\"\"\"\n\nMake sure each question is clear and the correctAnswer index is between 0 and 3.`;
`;
};

const runQuizGeneration = async ({ text, topic, category, count }) => {
  const prompt = buildQuizPrompt({ text, topic, category, count });
...
const generateQuizFromText = async (userId, requestBody) => {
  ...
  const aiPayload = await runQuizGeneration({ text: studyText, topic, category, count });
